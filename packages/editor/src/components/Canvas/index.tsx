...
import {
  ...
  useOperation,
  useStore,
} from '@shotz/shared/hooks';
import { Bounds, CanvasContexts, HistoryItemType, Point } from '@shotz/shared';
import { drawCrispImage, copyCanvasRegion } from '@shotz/shared/utils';
...
const Canvas = forwardRef<CanvasContexts>(
  function ScreenshotsCanvas(props, ref): ReactElement | null {
    ...
    const onCanvasPanelMouseDown = useCallback(
      (e: React.MouseEvent) => {
    ...
    const onCanvasPanelDoubleClick = useCallback(
      (e: React.MouseEvent) => {
        if (e.button !== 0 || !operation) {
          return;
        }
    ...
    useImperativeHandle<CanvasContexts, CanvasContexts>(ref, () => ({
